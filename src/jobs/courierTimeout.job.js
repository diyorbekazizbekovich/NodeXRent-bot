...
function startCourierTimeoutJob() {
  cron.schedule("*/1 * * * *", async () => {
    try {
      const timeoutThreshold = new Date(Date.now() - env.COURIER_RESPONSE_TIMEOUT_MINUTES * 60 * 1000);
      ...
      for (const order of stuckOrders) {
        ...
        logger.info(`Buyurtma #${order.id} kuryer kutish vaqti oshdi`, { context: "CourierTimeoutJob" });
      }
    } catch (err) {
      logger.error("CourierTimeoutJob xatoligi", { context: "CourierTimeoutJob", error: err.message });
    }
  });
}
